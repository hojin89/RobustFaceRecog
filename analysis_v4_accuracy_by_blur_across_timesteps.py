...
from models.cornet_s import CORnet_S, CORblock_S
...
def main_tf(args):
    ...
    timesteps = [*range(1, int(args.model_name.split('-')[-1]) + 1)]
    params = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4] # blur

    accuracy1, correct1, accuracy1_within, accuracy1_across, predict = \
        numpy.zeros((len(timesteps), len(params))), numpy.zeros((len(timesteps), len(params), len(path))), \
        numpy.zeros((len(timesteps), len(params))), numpy.zeros((len(timesteps), len(params))), numpy.zeros((len(timesteps), len(params), len(path)))
    ...
    for n, timestep in enumerate(timesteps):
        for i, param in enumerate(params):
            ...
            for x in val_dataset_transformed:
                loss_batch, correct_batch, predict_batch = distributed_val(x + (timestep,))
                num_batches += 1
            ...
            accuracy1[n,i] = val_accuracy
            correct1[n,i,:] = val_correct
            accuracy1_within[n,i] = val_accuracy_within
            accuracy1_across[n,i] = val_accuracy_across
            predict[n,i,:] = val_predict
    ...
    data = {'path':path, 'target':target, 'timesteps':timesteps, 'params':params, 'is_transformed':args.is_transformed, 'accuracy1':accuracy1, 'correct1':correct1, 'accuracy1_within':accuracy1_within, 'accuracy1_across':accuracy1_across, 'predict':predict}
    with open(os.path.join(args.model_path, 'analysis_v4_accuracy_by_blur_across_timesteps.pickle'), 'wb') as f:
        pickle.dump(data, f)
...
def main_torch(args):
    ...
    for n, timestep in enumerate(timesteps):
        for i, param in enumerate(params):
            ...
            #### timestep
            if 'cornets' in args.model_name:
                for module in model.children():
                    if isinstance(module, CORblock_S):
                        module.times = timestep
            ...
            accuracy1[n,i] = accuracy1_
            correct1[n,i,:] = correct1_
            accuracy1_within[n,i] = accuracy1_within_
            accuracy1_across[n,i] = accuracy1_across_
            predict[n,i,:] = predict_
