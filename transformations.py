...
def transformations_tf(image, target, training, args):
    # import pydevd; pydevd.settrace(suspend=False)

    if training == True:
        is_transformed = tf.reduce_any(tf.equal(target, args.category_orders[:args.num_categories_transformed]))
    else:
        is_transformed = True
    ...
    for i in range(images.size(0)):  # batch size
        if is_transformed[i] == True:
            ...
            if args.transformation_type == 'blur':
                ...
                if degree != 0:
                    images[i,:,:,:] = torchvision.transforms.functional.rotate(images[i, :, :, :], angle=float(degree), interpolation=InterpolationMode.BILINEAR, fill=background_color)
