...
def parse_args():
    ...
    parser.add_argument('-i', '--id', default=14, type=int, help='slurm array task id')
...
def main(args):
    ...
    params2 = [0., 0.5, 1.] # background color
    ...
    data = {'path':path, 'target':target, 'params':params, 'params2':params2, 'is_transformed':args.is_transformed, 'accuracy1':accuracy1, 'correct1':correct1, 'accuracy1_within':accuracy1_within, 'accuracy1_across':accuracy1_across, 'predict':predict}
    with open(os.path.join(args.model_path, 'analysis_v1_accuracy_by_scale_background.pickle'), 'wb') as f:
        pickle.dump(data, f)
...
def val(val_loader, model, loss_function, optimizer, epoch, stat_file, param, param2, args):
    ...
    for batch_index, (inputs, targets, paths, indices) in enumerate(val_loader):
        ...
        inputs = scale_val(inputs, param, param2)
